function draw_characteristics_of_burgers_equation_with_sin_initial(N::Int)
    ...
    function evaluate(t::Float64, x::Float64, tol::Float64=1e-16, maxiter::Int=1000)
        function evaluate_in_0_to_pi(t::Float64, x::Float64, tol::Float64=1e-16, maxiter::Int=1000)
            # find nearest characteristics
            d = zeros(Float64, N + 1)
            for i = 1:N+1
                xₚ = list_k[i] * t + list_x₀[i]
                d[i] = xₚ <= π ? abs(xₚ - x) : 2π
            end
...
function exact_solver(x, t, α, β)
    u = zeros(Float64, size(x))
    for i in eachindex(x)
        v = evaluate(β * t, x[i] - α * t)
        u[i] = β * v + α
    end
    u
end